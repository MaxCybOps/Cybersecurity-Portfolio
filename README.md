# 🛡️ Cybersecurity Portfolio — Ogbodo Uchenna Maxwell Adrian

<p align="center">
  <img src="https://img.shields.io/badge/Penetration_Testing-Critical-red?style=for-the-badge&logo=kali-linux&logoColor=white"/>
  <img src="https://img.shields.io/badge/Vulnerability_Assessment-orange?style=for-the-badge&logo=owasp&logoColor=white"/>
  <img src="https://img.shields.io/badge/SOC_Operations-blue?style=for-the-badge&logo=splunk&logoColor=white"/>
  <img src="https://img.shields.io/badge/OSINT-purple?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/MITRE_ATT%26CK-darkred?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Incident_Response-green?style=for-the-badge"/>
</p>

<p align="center">
  <b>20 documented engagements across penetration testing, vulnerability assessment, OSINT, SOC operations, APT simulation, incident response, and security research.</b>
</p>
...
|---|-----------|------|-------------|-------|
| 01 | [bWAPP Vulnerability Assessment](#01-bwapp-vulnerability-assessment) | VAPT | Digitanotion Ltd | OWASP ZAP, Manual |
| 02 | [MySQL Server Vulnerability Assessment](#02-mysql-server-vulnerability-assessment) | Risk Assessment | E-commerce Co. | NIST SP 800-30 |
| 03 | [Botium Toys Risk Assessment](#03-botium-toys-risk-assessment) | Security Audit | Botium Toys | NIST CSF |
| 04 | [DVWA Web Application Exploitation](#04-dvwa-web-application-exploitation) | Pen Testing | EvolveHQ | Burp Suite, Manual |
| 05 | [EternalBlue MS17-010 Exploitation](#05-eternalblue-ms17-010-exploitation) | Pen Testing | EvolveHQ / TryHackMe | Nmap, Metasploit |
| 06 | [DVWA Brute Force — 3 Security Levels](#06-dvwa-brute-force--three-security-levels) | Web App Testing | EvolveHQ | Burp Suite Intruder |
| 07 | [Operation Shadow Strike — APT Simulation](#07-operation-shadow-strike--apt-simulation) | Red Team / APT | ICDFA | Metasploit, SET, hping3, Nmap |
| 08 | [Operation AEGIS SHIELD — Cyber Command Exercise](#08-operation-aegis-shield--cyber-command-exercise) | Blue Team / C2 | ICDFA | MITRE ATT&CK, OODA |
| 09 | [OSINT Challenge — Investigating the Unknown](#09-osint-challenge--investigating-the-unknown) | OSINT | Digitanotion Ltd | Google Dorking, Yandex, TinEye |
| 10 | [TechCrush SOC Bootcamp](#10-techcrush-soc-bootcamp) | SOC / Blue Team | TechCrush | Wireshark, Splunk, Wazuh |
| 11 | [Incident Response Playbooks](#11-incident-response-playbooks) | IR Planning | TechCrush | NIST IR Framework |
| 12 | [Remote Hustle Security Implementation](#12-remote-hustle-security-implementation) | Security Engineering | Remote Hustle | Policy Design, RBAC |
| 13 | [OWASP Top 10 — Digitanotion Defence](#13-owasp-top-10--digitanotion-defence) | Security Research | Digitanotion Ltd | OWASP, CVE Research |
| 14 | [Wireshark Network Traffic Analysis](#14-wireshark-network-traffic-analysis) | Network Analysis | TechCrush Lab | Wireshark, ping, ARP |
| 15 | [Data Encryption, Hashing & Secure Transfer](#15-data-encryption-hashing--secure-transfer) | Cryptography Lab | TechCrush | GPG, SHA256, SCP, SSH |
| 16 | [File Permissions in Linux](#16-file-permissions-in-linux) | Linux Security | Google Cybersecurity | chmod, ls -la |
| 17 | [SQL Filters for Security Investigations](#17-sql-filters-for-security-investigations) | Security Analytics | Google Cybersecurity | SQL, MySQL |
| 18 | [Kali Linux Bash Automation](#18-kali-linux-bash-automation) | Scripting | Independent | Bash, chmod |
| 19 | [MITRE Frameworks Deep Dive](#19-mitre-frameworks-deep-dive) | Security Research | ICDFA | ATT&CK, D3FEND, CAPEC, CVE, CWE |
| 20 | [Cloud Security — AWS Assessment](#20-cloud-security--aws-assessment) | Cloud Security | Independent | AWS IAM, MFA |

---

## 01. bWAPP Vulnerability Assessment

**Organization:** Digitanotion Limited | **Role:** VAPT Intern | **Date:** December 2024
**Supervisor:** Mr. Divine Ezelibe (CISO) | **Ticket:** RT09219
...
|--------------|------|------|-----------------|
| SQL Injection (`/login.php`, `/admin.php`) | High | 7.5 | OWASP ZAP + Manual |
...
| Hidden File Found (`/phpinfo.php`) | Medium | — | ZAP Active Scan |

**Total:** 17 alerts — 0 High (post-manual), 6 Medium, 5 Low, 6 Informational

### Methodology
1. Deployed bWAPP on Docker, configured OWASP ZAP 2.15.0 as proxy
2. Ran traditional spider + AJAX spider against `http://127.5.0.1`
3. Executed automated active scan (4,394 requests, 146 new alerts identified)
4. Supplemented with manual testing on `/login.php` and `/admin.php`
5. Scored findings using CVSS v3.1 and compiled formal report with remediation
...
## 02. MySQL Server Vulnerability Assessment

**Context:** Newly hired security analyst at an e-commerce company | **Date:** January 2024
...
### Compliant Controls (Already in Place)
...
## 04. DVWA Web Application Exploitation
...
- Endpoint: `/login.php`
- Tool: Burp Suite Intruder (Cluster Bomb attack)
- Result: Recovered credentials — `admin:password`, `gordonb:abc123`, `smithy:password`, `pablo:letmein`, `1337:charley`
- Root Cause: No rate limiting, account lockout, or CAPTCHA
- Mitigation: Rate limiting, lockout after 5 failed attempts, CAPTCHA

**Command Injection**
- Payloads: `; whoami`, `; cat /etc/passwd`, `; ls`
- Root Cause: User input passed directly to system shell without sanitization
- Mitigation: `escapeshellarg()`, input whitelisting, disable dangerous PHP functions
...
- Payload: `<script>alert('XSS')</script>` in comment fields and search forms
- Root Cause: No output encoding — user input reflected directly in HTML
...
## 05. EternalBlue MS17-010 Exploitation

**Platform:** TryHackMe "Blue" Machine | **Organization:** EvolveHQ
**Vulnerability:** CVE-2017-0144 (EternalBlue — NSA exploit, leaked by Shadow Brokers)
...
nmap -A <target_ip>
```
Open ports: 135, 139, 445 (SMB), 3389 (RDP), 49152-49157 (RPC)

**2. Service Detection**
```bash
nmap -p 445 --script smb-vuln* <target_ip>
```
Result: SMBv1 enabled — vulnerable to MS17-010 (EternalBlue)
...
## 06. DVWA Brute Force — Three Security Levels
...
- Recovered: `gordonb:abc123`, `admin:password`, `smithy:password`, `pablo:letmein`, `1337:charley`
...
- Result: `admin:password` still recovered
- Lesson: CSRF tokens prevent cross-site attacks but not same-session brute force without additional controls like account lockout

---

## 07. Operation Shadow Strike — APT Simulation

**Organization:** ICDFA | **Course:** ACW202 — Offensive Cyber Operations
**Registration:** 2025/ACW/5607 | **Framework:** MITRE ATT&CK
...
| Phase | Control 1 | Control 2 | Control 3 |
|-------|-----------|-----------|-----------|
...
## 08. Operation AEGIS SHIELD — Cyber Command Exercise
...
|------------|-------|-------------|
| 08:30 | Suspicious Excel attachment flagged — `Q4_Manifests_Urgent.xlsm` | ANALYZE attachment; MONITOR logistics workstations |
| 09:15 | EDR alert: `powershell.exe` spawned by `EXCEL.EXE` on `EMP-WS-1138`, connecting to `185.141.25.234` | ISOLATE HOST; BLOCK IP; REQUEST FORENSICS |
...
| 10:30 | Lateral movement detected — host enumerating network shares, attempting access to `CM-DB-01` | ISOLATE HOST immediately; PROTECT critical asset |
| 11:30 | MRE extracts 3 hardcoded C2 domains from IRON VORTEX binary | BLOCK all 3 domains; FINAL SITREP to OPCOM-CYBER |

### IRON VORTEX C2 Infrastructure (IOCs Extracted)
...
## 09. OSINT Challenge — Investigating the Unknown
...
| Target URL | `katzsdelicatessen.com/corned-beef` | Google Dork |
| Dork Payload | `site:katzsdelicatessen.com corned beef` | Self-constructed |

### Lessons Learned
- Precision dorking with `site:` operator dramatically narrows results
- Cross-referencing Yandex and TinEye provides different image origin perspectives
- All findings must be verified through at least two independent sources before reporting

---

## 10. TechCrush SOC Bootcamp
...
Each playbook includes a post-incident template capturing: timeline reconstruction, root cause, systems affected, detection gap analysis, and control improvements.

---

## 12. Remote Hustle Security Implementation
...
### 2. Two-Factor Authentication Deployment (Google Workspace)
...
## 13. OWASP Top 10 — Digitanotion Defence

**Organization:** Digitanotion Limited | **Reference:** Equifax 2017 Breach Case Study

### Priority Vulnerabilities Analyzed for Digitanotion
...
## 14. Wireshark Network Traffic Analysis

**Course:** EvolveHQ / TechCrush Lab | **Focus:** ICMP, ARP, MAC address behavior

### Part 1 — Local ICMP Analysis
```
ping 192.168.1.114
```
- Source MAC: Local NIC MAC address (confirmed via `ipconfig /all`)
- Destination MAC: Target PC's actual MAC address
- MAC Resolution: ARP broadcast → ARP reply → ARP table updated → ICMP sent

### Part 2 — Remote ICMP Analysis
...
## 15. Data Encryption, Hashing & Secure Transfer
...
scp plaintext.txt plaintext.txt.gpg hostname@<ip>:/destination/path
```

### Wireshark Analysis
- **Unencrypted file (SCP of plaintext.txt):** TCP stream shows readable content
...
chmod g-x drafts
# Result: drwx------ (only researcher2 can access drafts)
```

### Permission String Breakdown
```
-rw-rw-rw-  →  Regular file | Owner: rw | Group: rw | Others: rw
drwxr-xr-x  →  Directory | Owner: rwx | Group: r-x | Others: r-x
```

---

## 17. SQL Filters for Security Investigations
...
SQL filtering is foundational for SOC analysts — used daily for log analysis, threat hunting, user behavior analytics, and access control audits. These queries demonstrate the ability to isolate anomalous events from large datasets efficiently.

---

## 18. Kali Linux Bash Automation
...
Full script available in: `07-linux-networking-labs/Kali-Bash-Automation/project_setup.sh`

---

## 19. MITRE Frameworks Deep Dive
...
## 20. Cloud Security — AWS Assessment
...
<p align="center">
  <i>"Security is not a product, it is a process — and this portfolio reflects that process in action."</i><br><br>
  <b>Ogbodo Uchenna Maxwell Adrian | @MaxCybOps</b>
</p>
